...
function buildTimeline(req) {
  return [
    { label: "Submission",      desc: "Request submitted and logged.",                                                                               time: formatDateTime(req.created_at),  done: true },
    { label: "Work Assigned",   desc: req.assigned_technician_id ? "A technician has been assigned." : "Not yet assigned.",                          time: req.assigned_technician_id ? formatDateTime(req.updated_at) : "—", done: !!req.assigned_technician_id },
    { label: "Work In Progress",desc: (req.status === "In Progress" || req.status === "Completed") ? "Technician is actively working." : "Not yet started.", time: (req.status === "In Progress" || req.status === "Completed") ? formatDateTime(req.updated_at) : "—", done: req.status === "In Progress" || req.status === "Completed" },
    { label: "Completed",       desc: req.status === "Completed" ? "Resolved and verified." : "Pending resolution.",                                time: req.status === "Completed" ? formatDateTime(req.updated_at) : "—", done: req.status === "Completed", final: true },
  ];
}
  ...
  return (
    <nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 60, background: C.white, borderTop: `1px solid ${C.outlineVariant}`, display: "flex", height: 62 }}>
      {NAV_ITEMS.map((item) => {
        const isActive = currentPath.startsWith(item.path);
        return (
          <button key={item.label} onClick={() => onNavigate(item.path)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, background: "none", border: "none", cursor: "pointer", color: isActive ? C.primaryContainer : C.onSurfaceVariant, fontFamily: MONO, fontSize: 9, letterSpacing: "0.04em", fontWeight: isActive ? 700 : 400, borderTop: isActive ? `2px solid ${C.primaryContainer}` : "2px solid transparent" }}>
  ...
  async function handleSubmit() {
    ...
    try {
      const { data, error: insertError } = await supabase
        .from("requests")
        .insert({ title: form.title.trim(), description: form.description.trim() || null, category: form.category, priority: form.priority, location: form.location.trim(), department: form.department.trim() || null, status: "Pending", created_by: userId })
        .select().single();
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: "min(520px, 95vw)", background: C.white, borderRadius: 20, boxShadow: "0 24px 64px rgba(0,0,0,0.22)", zIndex: 201, fontFamily: SANS, overflow: "hidden" }}>
        ...
        <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16, maxHeight: "58vh", overflowY: "auto" }}>
          ...
          <div style={{ display: "flex", gap: 10, padding: "10px 14px", background: C.surfaceContainerLow, border: `1px solid ${C.outlineVariant}`, borderRadius: 8 }}>
            ...
            <p style={{ margin: 0, fontSize: 12, color: C.onSurfaceVariant, lineHeight: 1.5 }}>A technician will be assigned by the administrator. You'll receive a notification once your request is reviewed.</p>
          </div>
        </div>
...
export default function StudentDashboard() {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("requests")
        .select("id, title, description, category, priority, status, location, department, created_by, assigned_technician_id, created_at, updated_at")
  ...
  useEffect(() => {
    if (!user?.id) return;
    const channel = supabase.channel("student-own-requests")
  ...
  async function handleCancel(id) {
    try {
      const { error } = await supabase.from("requests").update({ status: "Cancelled" }).eq("id", id).eq("created_by", user.id);
      if (error) throw error;
  ...
  const activeReq  = requests.find(r => r.status !== "Completed" && r.status !== "Cancelled") ?? requests[0] ?? null;
