  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: 'var(--color-surface-container-lowest)', zIndex: 101, boxShadow: "-6px 0 32px rgba(0,0,0,0.12)",
        ...
        <main style={{ flex: 1, padding: isMobile ? "20px 14px 80px" : "32px", maxWidth: 1600, width: "100%", alignSelf: "center" }}>
          ...
          <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", justifyContent: "space-between", alignItems: isMobile ? "flex-start" : "flex-end", marginBottom: isMobile ? 20 : 28, gap: 16 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, flexShrink: 0 }}>
              {STAT_CARDS.map((s) => (
                <div key={s.label} style={{ background: 'var(--color-surface-container-lowest)', border: `1px solid ${C.outlineVariant}`, borderRadius: 12, padding: "12px 14px", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {isMobile && (
            <button onClick={() => setShowFilters((p) => !p)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", background: 'var(--color-surface-container-lowest)', border: `1px solid ${C.outlineVariant}`, borderRadius: 8, cursor: "pointer", fontSize: 12, fontFamily: MONO, fontWeight: 700, color: C.onSurface, marginBottom: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "260px 1fr", gap: isMobile ? 16 : 28, alignItems: "start" }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              {loading ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  {[1, 2, 3].map((i) => (
                    <div key={i} style={{ height: 120, background: 'var(--color-surface-container-lowest)', border: `1px solid ${C.outlineVariant}`, borderRadius: 14, animation: "pulse 1.5s ease-in-out infinite", marginLeft: isMobile ? 0 : 48 }} />
                  ))}
                </div>
              ) : Object.keys(grouped).length === 0 ? (
                <div style={{ padding: "56px 24px", textAlign: "center", background: 'var(--color-surface-container-lowest)', borderRadius: 14, border: `1px solid ${C.outlineVariant}`, marginLeft: isMobile ? 0 : 48 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                  {Object.entries(grouped).map(([month, items]) => (
                    <div key={month} style={{ marginBottom: 28 }}>
                      {/* Month label */}
                      <div style={{ position: "sticky", top: 64, zIndex: 20, paddingBottom: 14, paddingLeft: isMobile ? 0 : 48, background: `color-mix(in srgb, var(--color-surface) 88%, transparent)`, backdropFilter: "blur(8px)" }}>
                        ...
                        {items.map((item) => (
                          isMobile ? (
                            ...
                            <div key={item.id} onClick={() => setSelected(item)} style={{ cursor: "pointer" }}>
                              <div style={{ background: 'var(--color-surface-container-lowest)', borderRadius: 14, padding: "16px 18px", border: `1px solid ${C.outlineVariant}` }}>
                  ...
                  {filtered.length > visibleCount && (
                    <div style={{ paddingLeft: isMobile ? 0 : 48, paddingTop: 8 }}>
                      <button onClick={() => setVisibleCount((p) => p + 6)} style={{
                        ...
                        background: 'var(--color-surface-container-lowest)', border: `2px dashed ${C.outlineVariant}`,
