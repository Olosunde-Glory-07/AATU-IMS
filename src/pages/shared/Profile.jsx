...
// ─── Role theming ─────────────────────────────────────────────────────────────
// Each role gets its own accent color so the page feels native to that portal
...
export default function Profile({ role: roleProp }) {
  ...
  // role can be passed explicitly, or inferred from the logged-in profile
  ...
  // ── Notification preferences (local UI state — wire to profiles table if you add columns for these) ──
  ...
  // ── Avatar upload — real Supabase Storage upload ───────────────────────────
  ...
  // ── Save name/phone — real Supabase update ──────────────────────────────────
  ...
  // ── Change password — real Supabase Auth call ───────────────────────────────
  async function handleChangePassword(e) {
    ...
    setChangingPw(true)
    try {
      // Supabase doesn't verify "current password" server-side before an update,
      // so we re-authenticate first to confirm the user actually knows it.
  ...
  return (
    <main className="flex-1 min-h-screen bg-[#f9f9ff]">
      ...
      <header className="flex justify-between items-center h-16 px-4 sm:px-6 bg-[#f9f9ff] border-b border-[#dcc0bd] sticky top-0 z-40">
        <div className="flex items-center gap-3 min-w-0">
          <button
            ...
            className="p-2 -ml-2 text-[#554240] hover:bg-[#f0f3ff] rounded-full transition-colors flex-shrink-0"
          >
        ...
        <span
          className="text-xs font-mono px-3 py-1 rounded-full font-bold flex-shrink-0"
      ...
      <div className="max-w-3xl mx-auto p-4 sm:p-8 space-y-6">
          ...
          <div className="flex items-center gap-5 flex-wrap">
            <div className="relative flex-shrink-0">
              <div
                ...
                style={{ borderColor: theme.light, background: theme.light }}
              >
                {avatarUrl ? (
            ...
            <div className="flex-1 min-w-[180px]">
              ...
              <div className="flex gap-2 mt-3">
                <button
                  onClick={() => fileInputRef.current?.click()}
                  disabled={uploading}
                  className="px-3 py-1.5 text-xs font-mono border border-[#dcc0bd] rounded-lg hover:bg-[#f0f3ff] transition-colors disabled:opacity-50"
                >
                  Upload new photo
                </button>
                {avatarUrl && (
                  <button
                    ...
                    disabled={uploading}
                    className="px-3 py-1.5 text-xs font-mono text-[#ba1a1a] border border-[#ffdad6] bg-[#ffdad6]/30 rounded-lg hover:bg-[#ffdad6] transition-colors disabled:opacity-50"
                  >
                    Remove
                  </button>
                )}
          ...
          <form onSubmit={handleSaveProfile} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  value={user?.email ?? ''}
                  disabled
                  className={`${inputStyle} bg-[#f0f3ff] text-[#554240] cursor-not-allowed`}
                />
                <p className="text-[11px] text-[#554240]/60 mt-1">Email cannot be changed here. Contact admin.</p>
              </div>
              <div>
            ...
            <div className="flex justify-end pt-1">
              <button
                ...
                className="px-5 py-2.5 rounded-lg text-sm font-mono font-bold disabled:opacity-50 transition-opacity"
        ...
        <section className="bg-white border border-[#dcc0bd] rounded-xl p-6">
          <h3 className="text-base font-bold text-[#151c27] mb-4">Notification Preferences</h3>
          <div className="space-y-4">
            {[
              { label: 'Email Notifications', desc: 'Receive updates about your requests via email.', value: emailNotifs, set: setEmailNotifs },
              { label: 'Push Notifications',  desc: 'Receive in-app alerts for urgent updates.',        value: pushNotifs,  set: setPushNotifs  },
            ].map(({ label, desc, value, set }) => (
              <div key={label} className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-medium text-[#151c27]">{label}</p>
                  <p className="text-xs text-[#554240]">{desc}</p>
                </div>
                <button
                  onClick={() => { set(v => !v); showToast(`${label} ${!value ? 'enabled' : 'disabled'}.`) }}
                  className="relative w-11 h-6 rounded-full transition-colors flex-shrink-0"
                  style={{ background: value ? theme.accent : '#dcc0bd' }}
                >
                  <span
          ...
          <p className="text-[11px] text-[#554240]/60 mt-4">
            These preferences are stored locally for now — add corresponding columns to your <code>profiles</code> table to persist them.
          </p>
        </section>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setConfirmLogoutAll(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-3">
              ...
              <button onClick={() => setConfirmLogoutAll(false)} className="p-1 hover:bg-[#f0f3ff] rounded-full transition-colors">
                <X size={18} />
              </button>
            </div>
            <p className="text-sm text-[#554240] mb-5">
              You'll be signed out on this device and any other device currently logged in to your account.
            </p>
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-2.5 bg-[#ba1a1a] text-white rounded-lg text-sm font-mono font-bold hover:opacity-90 transition-opacity"
              >
                Log Out Everywhere
              </button>
            </div>
      ...
      {/* ── Toast ─────────────────────────────────────────────────────────── */}
      {toast && (
        <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-[60] px-6 py-3 rounded-full text-sm font-mono shadow-xl flex items-center gap-2 whitespace-nowrap ${
