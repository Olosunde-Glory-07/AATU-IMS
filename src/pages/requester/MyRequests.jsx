...
function buildTimeline(r) {
  ...
  const steps = [
    { label: "Request Submitted", desc: "Your ticket has been generated and sent to your assigned monitor for review.", time: submittedTime, state: "done" },
  ];
  ...
  steps.push({ label: "Monitor Approved", desc: "Your monitor approved this request. Awaiting admin assignment.", time: "Done", state: "done" });
...
// ─── Top Bar ──────────────────────────────────────────────────────────────────
function TopBar({ search, setSearch, onNew, isMobile, firstName }) {
  return (
    <header style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        {!isMobile && (
          <>
            ...
            <div style={{ width: 36, height: 36, borderRadius: "50%", background: C.primaryFixedDim, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "#4a0404", fontSize: 14 }}>
              {firstName[0]}
            </div>
          </>
        )}
        <button onClick={onNew} style={{
          background: "#4a0404", color: "#ffffff", border: "none", cursor: "pointer",
...
function SubmitModal({ onClose, onSubmit }) {
  ...
  async function handleSubmit() {
    ...
    if (!monitorId) {
      setError("Please assign a staff monitor to review this request.");
      return;
    }
    if (!user) {
    ...
    <>
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: "min(520px,95vw)", background: CARD, borderRadius: 20, boxShadow: "0 24px 64px rgba(0,0,0,0.22)", zIndex: 201, fontFamily: SANS, overflow: "hidden" }}>
        ...
        <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16, maxHeight: "60vh", overflowY: "auto" }}>
          ...
          <div>
            ...
            <select value={monitorId} onChange={(e) => setMonitorId(e.target.value)} style={{ ...inp, cursor: "pointer" }} disabled={loadingStaff}>
              ...
              {staffOptions.map((s) => (
                <option key={s.id} value={s.id}>{s.full_name}{s.department ? ` — ${s.department}` : ""}</option>
              ))}
            </select>
          </div>
          <div style={{ display: "flex", gap: 10, padding: "10px 14px", background: C.surfaceContainerLow, border: `1px solid ${C.outlineVariant}`, borderRadius: 8 }}>
            ...
            <p style={{ margin: 0, fontSize: 12, color: C.onSurfaceVariant, lineHeight: 1.5 }}>
              Your assigned staff monitor reviews this request first. Once approved, an administrator assigns a technician and you'll be notified at each step.
            </p>
          </div>
        </div>
        <div style={{ padding: "14px 24px", borderTop: `1px solid ${C.outlineVariant}`, display: "flex", justifyContent: "flex-end", gap: 10, background: C.surfaceContainerLow }}>
          ...
          <button onClick={handleSubmit} disabled={submitting} style={{ padding: "9px 22px", background: "#4a0404", color: "#ffffff", border: "none", borderRadius: 8, cursor: submitting ? "not-allowed" : "pointer", fontSize: 12, fontFamily: MONO, fontWeight: 700, display: "flex", alignItems: "center", gap: 6, opacity: submitting ? 0.7 : 1 }}>
            <Icon name="send" size={14} style={{ color: "#ffffff" }} />
    ...
    <>
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: "min(520px,95vw)", background: CARD, borderRadius: 20, boxShadow: "0 24px 64px rgba(0,0,0,0.22)", zIndex: 201, fontFamily: SANS, overflow: "hidden" }}>
        <div style={{ padding: "20px 24px 16px", borderBottom: `1px solid ${C.outlineVariant}`, background: C.errorContainer, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", padding: 6, color: C.onErrorContainer, display: "flex" }}>
            <Icon name="close" size={20} />
          </button>
        </div>
        ...
        <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16, maxHeight: "60vh", overflowY: "auto" }}>
          ...
          <div>
            <label style={lbl}>Location *</label>
            <input value={form.location} onChange={set("location")} style={inp} />
          </div>
          <div>
            <label style={lbl}>Department</label>
            <input value={form.department} onChange={set("department")} style={inp} />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div>
              <label style={lbl}>Category</label>
          ...
          <div>
            <label style={lbl}>Description</label>
            <textarea value={form.description} onChange={set("description")} rows={3} style={{ ...inp, resize: "vertical", lineHeight: 1.6 }} />
          </div>
          <div>
            <label style={lbl}>Assign Monitor (Staff) *</label>
            <select value={monitorId} onChange={(e) => setMonitorId(e.target.value)} style={{ ...inp, cursor: "pointer" }} disabled={loadingStaff}>
        ...
        <div style={{ padding: "14px 24px", borderTop: `1px solid ${C.outlineVariant}`, display: "flex", justifyContent: "flex-end", gap: 10, background: C.surfaceContainerLow }}>
          <button onClick={onClose} style={{ padding: "9px 20px", border: `1px solid ${C.outlineVariant}`, borderRadius: 8, background: "none", cursor: "pointer", fontSize: 12, fontFamily: MONO, color: C.onSurface }}>Cancel</button>
          <button onClick={handleResubmit} disabled={submitting} style={{ padding: "9px 22px", background: "#4a0404", color: "#ffffff", border: "none", borderRadius: 8, cursor: submitting ? "not-allowed" : "pointer", fontSize: 12, fontFamily: MONO, fontWeight: 700, display: "flex", alignItems: "center", gap: 6, opacity: submitting ? 0.7 : 1 }}>
            <Icon name="send" size={14} style={{ color: "#ffffff" }} />
    ...
    <>
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: "min(440px,95vw)", background: CARD, borderRadius: 16, boxShadow: "0 20px 60px rgba(0,0,0,0.18)", zIndex: 201, fontFamily: SANS, overflow: "hidden" }}>
        ...
        <div style={{ padding: "12px 20px", borderTop: `1px solid ${C.outlineVariant}`, display: "flex", justifyContent: "flex-end", gap: 10, background: C.surfaceContainerLow }}>
          ...
          <button onClick={() => { if (note.trim()) { onSave(note.trim()); onClose(); } }} disabled={!note.trim()} style={{ padding: "8px 18px", background: "#4a0404", color: "#ffffff", border: "none", borderRadius: 8, cursor: note.trim() ? "pointer" : "not-allowed", fontSize: 12, fontFamily: MONO, fontWeight: 700, opacity: note.trim() ? 1 : 0.5 }}>
            Save Note
          </button>
        </div>
      </div>
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: "min(400px,95vw)", background: CARD, borderRadius: 16, boxShadow: "0 20px 60px rgba(0,0,0,0.18)", zIndex: 201, fontFamily: SANS, overflow: "hidden" }}>
        <div style={{ padding: 24 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button onClick={onConfirm} style={{ flex: 1, padding: "10px 0", background: C.error, color: "#ffffff", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 13, fontFamily: MONO, fontWeight: 700 }}>Delete</button>
          </div>
        </div>
      </div>
    </>
    ...
    <div style={wrapper}>
      ...
      <div style={{ padding: "24px 28px 20px", borderBottom: `1px solid ${C.outlineVariant}` }}>
        ...
        {isRejected && (
          <div style={{ marginTop: 14, padding: "14px 16px", background: C.errorContainer, borderRadius: 10, border: `1px solid color-mix(in srgb, ${C.error} 20%, transparent)`, display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <p style={{ margin: 0, fontSize: 13, color: C.onErrorContainer, lineHeight: 1.5 }}>
              {req.monitorRejectionReason || "No specific reason was given."}
            </p>
            <button
              onClick={onResubmit}
              style={{ alignSelf: "flex-start", display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", background: C.error, color: "#ffffff", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 12, fontFamily: MONO, fontWeight: 700 }}
            >
              <Icon name="edit" size={15} style={{ color: "#ffffff" }} />
              Edit & Resubmit
            </button>
          </div>
        )}
              ...
              return (
                <div key={i} style={{ display: "flex", gap: 16, position: "relative", paddingBottom: i < req.timeline.length - 1 ? 22 : 0 }}>
                  ...
                  <div style={{ width: 24, height: 24, borderRadius: "50%", flexShrink: 0, zIndex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: dotBg, border: `2px solid ${dotBorder}`, boxShadow: isActive ? "0 0 0 4px #fef3c7" : "none" }}>
                    {isDone && <Icon name="check" size={13} style={{ color: "#ffffff" }} />}
                    {isNote && <Icon name="notes" size={12} style={{ color: "#ffffff" }} />}
      ...
      <div style={{ padding: "16px 28px", borderTop: `1px solid ${C.outlineVariant}`, display: "flex", gap: 10 }}>
        <div style={{ display: "flex", gap: 8, marginLeft: "auto" }}>
          ...
          {isRejected && (
            <button onClick={onResubmit} style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 16px", background: "#4a0404", color: "#ffffff", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 12, fontFamily: MONO, fontWeight: 700 }}>
              <Icon name="edit" size={15} style={{ color: "#ffffff" }} />
              Edit & Resubmit
            </button>
          )}
...
export default function RequesterMyRequests() {
  ...
  return (
    <div style={{ background: C.surface, minHeight: "100%", fontFamily: SANS, color: C.onSurface }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", minHeight: "100%" }}>
        <TopBar search={search} setSearch={setSearch} onNew={() => setShowNew(true)}
          isMobile={isMobile} firstName={firstName} />
              ...
              {loading ? (
                ...
                <div style={{ padding: "48px 24px", textAlign: "center", background: CARD, borderRadius: 14, border: `1px solid ${C.outlineVariant}` }}>
                  ...
                  <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
                    ...
                    <button onClick={() => setShowNew(true)} style={{ padding: "7px 16px", background: "#4a0404", color: "#ffffff", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 12, fontFamily: MONO, fontWeight: 700 }}>
                      New Request
                    </button>
                  </div>
                </div>
